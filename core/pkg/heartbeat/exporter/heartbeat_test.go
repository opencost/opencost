...
func TestHeartbeatExporter(t *testing.T) {
	...
	files, _ := store.List(path.Join("", MockClusterId, heartbeat.HeartbeatEventName, MockApplicationName))
	if len(files) == 0 {
	...
	for _, f := range fileNames {
		fpath := filepath.Join("", MockClusterId, "heartbeat", MockApplicationName, f)
