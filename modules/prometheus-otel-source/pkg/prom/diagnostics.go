...
var diagnosticDefinitions = map[string]*diagnosticDefinition{
	...
	KSMDiagnosticMetricID: {
		...
		QueryFmt:    `absent_over_time(kube_pod_container_resource_requests{%s}[5m] %s)`,
		Label:       "Kube-state-metrics available (OTel)",
		Description: "Determine if kube_pod_container_resource_requests from OTel-scraped KSM (kube-state-metrics) is available during last 5 minutes. This is the primary metric used for container CPU/RAM request allocation.",
	...
	KSMVersionDiagnosticMetricID: {
		...
		Label:       "PV capacity metric available (OTel/KSM)",
		Description: "Determine if kube_persistentvolume_capacity_bytes from OTel-scraped KSM is present during last 5 minutes. Required for persistent volume cost allocation.",
	...
	// NOTE: CPU throttling diagnostic is intentionally omitted from the OTel module.
	// The cAdvisor metrics container_cpu_cfs_throttled_periods_total and container_cpu_cfs_periods_total
	// are not emitted by the OTel kubeletstats receiver. The upstream prometheus-source module
	// keeps this diagnostic for classic Prometheus setups with cAdvisor.
	//
	// CPUThrottlingDiagnosticMetricID is kept as a constant for API compatibility but
	// excluded from diagnosticDefinitions.
	...
	KSMCPUCapacityMetricID: {
		...
		QueryFmt:    `absent_over_time(kube_node_status_capacity{resource="cpu"%s}[5m] %s)`,
		Label:       "Node CPU capacity metric available (OTel/KSM)",
		Description: "Determine if kube_node_status_capacity{resource=\"cpu\"} from OTel-scraped KSM is available during the last 5 minutes. Required for node CPU capacity allocation.",
	},
	KSMAllocatableCPUCoresMetricID: {
		ID:          KSMAllocatableCPUCoresMetricID,
		QueryFmt:    `absent_over_time(kube_node_status_allocatable{resource="cpu"%s}[5m] %s)`,
		Label:       "Node allocatable CPU metric available (OTel/KSM)",
		Description: "Determine if kube_node_status_allocatable{resource=\"cpu\"} from OTel-scraped KSM is available during the last 5 minutes. Required for node CPU allocatable calculation.",
	},
}

// clusterFilterSuffix returns ",<filter>" when filter is non-empty, or "" when empty.
// Use this instead of embedding ClusterFilter directly after a comma in PromQL selectors,
// to avoid generating invalid PromQL like {resource="cpu",} when cluster filtering is off.
func clusterFilterSuffix(filter string) string {
	if filter == "" {
		return ""
	}
	return "," + filter
...
func GetPrometheusMetrics(client prometheus.Client, config *promsource.OpenCostPrometheusConfig, offset string) PrometheusDiagnostics {
	...
	for _, definition := range diagnosticDefinitions {
		pd := definition.NewDiagnostic(clusterFilterSuffix(config.ClusterFilter), offset)
