...
func (pds *PrometheusMetricsQuerier) QueryPods(start, end time.Time) *source.Future[source.PodsResult] {
	...
	return source.NewFuture(source.DecodePodsResult, pds.NewNamedContext(AllocationContextName).QueryAtTime(q, end))
}
...
func (pds *PrometheusMetricsQuerier) QueryRAMBytesAllocated(start, end time.Time) *source.Future[source.RAMBytesAllocatedResult] {
	...
	return source.NewFuture(source.DecodeRAMBytesAllocatedResult, pds.NewNamedContext(AllocationContextName).QueryAtTime(q, end))
}
...
func (pds *PrometheusMetricsQuerier) QueryRAMRequests(start, end time.Time) *source.Future[source.RAMRequestsResult] {
	...
	return source.NewFuture(source.DecodeRAMRequestsResult, pds.NewNamedContext(AllocationContextName).QueryAtTime(q, end))
}
...
func (pds *PrometheusMetricsQuerier) QueryRAMUsageAvg(start, end time.Time) *source.Future[source.RAMUsageAvgResult] {
	...
	q := fmt.Sprintf(`avg(avg_over_time(container_memory_working_set{k8s_container_name!="",k8s_container_name!="POD",%s}[%s])) by (k8s_container_name,pod_name,k8s_pod_name,k8s_namespace_name,k8s_node_name,instance,%s)`, cfg.ClusterFilter, d, cfg.ClusterLabel)
	log.Debugf(PrometheusMetricsQueryLogFormat, "QueryRAMUsageAvg", end.Unix(), q)
	return source.NewFuture(source.DecodeRAMUsageAvgResult, pds.NewNamedContext(AllocationContextName).QueryAtTime(q, end))
}
...
func (pds *PrometheusMetricsQuerier) QueryRAMUsageMax(start, end time.Time) *source.Future[source.RAMUsageMaxResult] {
	...
	d := timeutil.DurationString(end.Sub(start))
	// OTel kubeletstats receiver uses container_memory_working_set (not k8s_container_memory_working_set)
	q := fmt.Sprintf(`max(max_over_time(container_memory_working_set{k8s_container_name!="",k8s_container_name!="POD",%s}[%s])) by (k8s_container_name,pod_name,k8s_pod_name,k8s_namespace_name,k8s_node_name,instance,%s)`, cfg.ClusterFilter, d, cfg.ClusterLabel)
	log.Debugf(PrometheusMetricsQueryLogFormat, "QueryRAMUsageMax", end.Unix(), q)
	return source.NewFuture(source.DecodeRAMUsageMaxResult, pds.NewNamedContext(AllocationContextName).QueryAtTime(q, end))
}
...
func (pds *PrometheusMetricsQuerier) QueryCPUCoresAllocated(start, end time.Time) *source.Future[source.CPUCoresAllocatedResult] {
	...
	return source.NewFuture(source.DecodeCPUCoresAllocatedResult, pds.NewNamedContext(AllocationContextName).QueryAtTime(q, end))
}
...
func (pds *PrometheusMetricsQuerier) QueryCPURequests(start, end time.Time) *source.Future[source.CPURequestsResult] {
	...
	return source.NewFuture(source.DecodeCPURequestsResult, pds.NewNamedContext(AllocationContextName).QueryAtTime(q, end))
}
...
func (pds *PrometheusMetricsQuerier) QueryCPUUsageAvg(start, end time.Time) *source.Future[source.CPUUsageAvgResult] {
	...
	q := fmt.Sprintf(`avg(rate(container_cpu_time{k8s_container_name!="",k8s_container_name!="POD",%s}[%s])) by (container_name,k8s_container_name,pod_name,k8s_pod_name,k8s_namespace_name,k8s_node_name,instance,%s)`, cfg.ClusterFilter, d, cfg.ClusterLabel)
	log.Debugf(PrometheusMetricsQueryLogFormat, "QueryCPUUsageAvg", end.Unix(), q)
	return source.NewFuture(source.DecodeCPUUsageAvgResult, pds.NewNamedContext(AllocationContextName).QueryAtTime(q, end))
}
...
func (pds *PrometheusMetricsQuerier) QueryCPUUsageMax(start, end time.Time) *source.Future[source.CPUUsageMaxResult] {
	...
	qRR := fmt.Sprintf(`max(max_over_time(kubecost_container_cpu_usage_irate{%s}[%s])) by (container_name,k8s_container_name,pod_name,k8s_pod_name,k8s_namespace_name,k8s_node_name,instance,%s)`, cfg.ClusterFilter, d, cfg.ClusterLabel)
	log.Debugf(PrometheusMetricsQueryLogFormat, queryName, end.Unix(), qRR)
	ctx := pds.NewNamedContext(AllocationContextName)
	resRR, _ := ctx.QueryAtTime(qRR, end).Await()
	if len(resRR) > 0 {
	...
	qSub := fmt.Sprintf(`max(max_over_time(irate(container_cpu_time{k8s_container_name!="POD",k8s_container_name!="",%s}[%dm])[%s:%dm])) by (k8s_container_name,pod_name,k8s_pod_name,k8s_namespace_name,k8s_node_name,instance,%s)`, cfg.ClusterFilter, 2*m, d2, m, cfg.ClusterLabel)
