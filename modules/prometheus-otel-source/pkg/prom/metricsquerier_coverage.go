...
func (pds *PrometheusMetricsQuerier) QueryDataCoverage(limitDays int) (time.Time, time.Time, error) {
	// OTel kubeletstats receiver uses container_memory_working_set
	startQuery := fmt.Sprintf(`min(min_over_time(container_memory_working_set{k8s_container_name!="POD",k8s_container_name!=""%s}[%dd]))`, pds.clusterFilterSuffix(), limitDays)

	// Query for the latest timestamp with data
	endQuery := fmt.Sprintf(`max(max_over_time(container_memory_working_set{k8s_container_name!="POD",k8s_container_name!=""%s}[%dd]))`, pds.clusterFilterSuffix(), limitDays)
