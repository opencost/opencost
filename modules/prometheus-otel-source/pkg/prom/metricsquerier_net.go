...
func (pds *PrometheusMetricsQuerier) QueryNetZoneGiB(start, end time.Time) *source.Future[source.NetZoneGiBResult] {
	...
	q := fmt.Sprintf(`sum(increase(kubecost_pod_network_egress_bytes_total{internet="false",same_zone="false",same_region="true",%s}[%s:%dm])) by (k8s_pod_name,k8s_namespace_name,%s) / 1024 / 1024 / 1024`, cfg.ClusterFilter, d, m, cfg.ClusterLabel)
...
func (pds *PrometheusMetricsQuerier) QueryNetRegionGiB(start, end time.Time) *source.Future[source.NetRegionGiBResult] {
	...
	m := cfg.DataResolutionMinutes
	d := pds.durationStringFor(start, end, m, false)
	q := fmt.Sprintf(`sum(increase(kubecost_pod_network_egress_bytes_total{internet="false",same_zone="false",same_region="false",%s}[%s:%dm])) by (k8s_pod_name,k8s_namespace_name,%s) / 1024 / 1024 / 1024`, cfg.ClusterFilter, d, m, cfg.ClusterLabel)
...
func (pds *PrometheusMetricsQuerier) QueryNetInternetGiB(start, end time.Time) *source.Future[source.NetInternetGiBResult] {
	...
	m := cfg.DataResolutionMinutes
	d := pds.durationStringFor(start, end, m, false)
	q := fmt.Sprintf(`sum(increase(kubecost_pod_network_egress_bytes_total{internet="true",%s}[%s:%dm])) by (k8s_pod_name,k8s_namespace_name,%s) / 1024 / 1024 / 1024`, cfg.ClusterFilter, d, m, cfg.ClusterLabel)
...
func (pds *PrometheusMetricsQuerier) QueryNetInternetServiceGiB(start, end time.Time) *source.Future[source.NetInternetServiceGiBResult] {
	...
	m := cfg.DataResolutionMinutes
	d := pds.durationStringFor(start, end, m, false)
	q := fmt.Sprintf(`sum(increase(kubecost_pod_network_egress_bytes_total{internet="true",%s}[%s:%dm])) by (k8s_pod_name,k8s_namespace_name,service,%s) / 1024 / 1024 / 1024`, cfg.ClusterFilter, d, m, cfg.ClusterLabel)
...
func (pds *PrometheusMetricsQuerier) QueryNetTransferBytes(start, end time.Time) *source.Future[source.NetTransferBytesResult] {
	...
	m := cfg.DataResolutionMinutes
	d := pds.durationStringFor(start, end, m, false)
	q := fmt.Sprintf(`sum(increase(k8s_pod_network_io{direction="transmit",k8s_pod_name!="",%s}[%s:%dm])) by (k8s_pod_name,k8s_namespace_name,%s)`, cfg.ClusterFilter, d, m, cfg.ClusterLabel)
...
func (pds *PrometheusMetricsQuerier) QueryNetZoneIngressGiB(start, end time.Time) *source.Future[source.NetZoneIngressGiBResult] {
	...
	m := cfg.DataResolutionMinutes
	d := pds.durationStringFor(start, end, m, false)
	q := fmt.Sprintf(`sum(increase(kubecost_pod_network_ingress_bytes_total{internet="false",same_zone="false",same_region="true",%s}[%s:%dm])) by (k8s_pod_name,k8s_namespace_name,%s) / 1024 / 1024 / 1024`, cfg.ClusterFilter, d, m, cfg.ClusterLabel)
...
func (pds *PrometheusMetricsQuerier) QueryNetRegionIngressGiB(start, end time.Time) *source.Future[source.NetRegionIngressGiBResult] {
	...
	m := cfg.DataResolutionMinutes
	d := pds.durationStringFor(start, end, m, false)
	q := fmt.Sprintf(`sum(increase(kubecost_pod_network_ingress_bytes_total{internet="false",same_zone="false",same_region="false",%s}[%s:%dm])) by (k8s_pod_name,k8s_namespace_name,%s) / 1024 / 1024 / 1024`, cfg.ClusterFilter, d, m, cfg.ClusterLabel)
...
func (pds *PrometheusMetricsQuerier) QueryNetInternetIngressGiB(start, end time.Time) *source.Future[source.NetInternetIngressGiBResult] {
	...
	m := cfg.DataResolutionMinutes
	d := pds.durationStringFor(start, end, m, false)
	q := fmt.Sprintf(`sum(increase(kubecost_pod_network_ingress_bytes_total{internet="true",%s}[%s:%dm])) by (k8s_pod_name,k8s_namespace_name,%s) / 1024 / 1024 / 1024`, cfg.ClusterFilter, d, m, cfg.ClusterLabel)
...
func (pds *PrometheusMetricsQuerier) QueryNetInternetServiceIngressGiB(start, end time.Time) *source.Future[source.NetInternetServiceIngressGiBResult] {
	...
	m := cfg.DataResolutionMinutes
	d := pds.durationStringFor(start, end, m, false)
	q := fmt.Sprintf(`sum(increase(kubecost_pod_network_ingress_bytes_total{internet="true",%s}[%s:%dm])) by (k8s_pod_name,k8s_namespace_name,service,%s) / 1024 / 1024 / 1024`, cfg.ClusterFilter, d, m, cfg.ClusterLabel)
...
func (pds *PrometheusMetricsQuerier) QueryNetReceiveBytes(start, end time.Time) *source.Future[source.NetReceiveBytesResult] {
	...
	m := cfg.DataResolutionMinutes
	d := pds.durationStringFor(start, end, m, false)
	q := fmt.Sprintf(`sum(increase(k8s_pod_network_io{direction="receive",k8s_pod_name!="",%s}[%s:%dm])) by (k8s_pod_name,k8s_namespace_name,%s)`, cfg.ClusterFilter, d, m, cfg.ClusterLabel)
