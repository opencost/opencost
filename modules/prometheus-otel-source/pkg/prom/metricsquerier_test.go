...
func TestQueryRAMBytesAllocated_UsesOTelLabels(t *testing.T) {
	...
	if !strings.Contains(q, "kube_pod_container_resource_requests") {
		t.Errorf("expected metric 'kube_pod_container_resource_requests' in query, got:\n%s", q)
	}
	if !strings.Contains(q, `resource="memory"`) {
		t.Errorf("expected resource=\"memory\" selector in query, got:\n%s", q)
	}
}
...
func TestQueryCPUUsageAvg_UsesOTelLabels(t *testing.T) {
	...
	if !strings.Contains(q, "container_cpu_usage") {
		t.Errorf("expected metric 'container_cpu_usage' in query, got:\n%s", q)
	}
	for _, label := range []string{"k8s_container_name", "k8s_pod_name", "k8s_namespace_name", "k8s_node_name"} {
		if !strings.Contains(q, label) {
			t.Errorf("expected OTel label %q in query, got:\n%s", label, q)
...
func TestQueryNodeCPUCoresAllocatable_UsesOTelLabels(t *testing.T) {
	...
	if !strings.Contains(q, "kube_node_status_allocatable") {
		t.Errorf("expected metric 'kube_node_status_allocatable' in query, got:\n%s", q)
	}
	if !strings.Contains(q, `resource="cpu"`) {
		t.Errorf("expected resource=\"cpu\" selector in query, got:\n%s", q)
	}
...
// --- P1: RAM/CPU Requests use OTel k8s_cluster receiver metrics ---

func TestQueryRAMRequests_UsesOTelMetric(t *testing.T) {
	spy := newSpyPromClient()
	querier := newTestQuerier(spy)
	q := captureQuery(t, spy, func() { querier.QueryRAMRequests(testStart, testEnd) })
...
func TestQueryCPURequests_UsesOTelMetric(t *testing.T) {
	spy := newSpyPromClient()
	querier := newTestQuerier(spy)
	q := captureQuery(t, spy, func() { querier.QueryCPURequests(testStart, testEnd) })
...
// --- P2: Pod active detection uses kube_pod_container_status_running ---

func TestQueryPods_UsesK8sPodPhase(t *testing.T) {
	spy := newSpyPromClient()
	querier := newTestQuerier(spy)
	q := captureQuery(t, spy, func() { querier.QueryPods(testStart, testEnd) })

	if !strings.Contains(q, "kube_pod_container_status_running") {
		t.Errorf("expected metric 'kube_pod_container_status_running' in query, got:\n%s", q)
	}
	if !strings.Contains(q, "== 1") {
		t.Errorf("expected '== 1' filter in query, got:\n%s", q)
	}
}

func TestQueryPodsUID_UsesK8sPodPhase(t *testing.T) {
	spy := newSpyPromClient()
	querier := newTestQuerier(spy)
	q := captureQuery(t, spy, func() { querier.QueryPodsUID(testStart, testEnd) })
...
// --- P3: Node capacity uses KSM kube_node_status_capacity/allocatable ---

func TestQueryNodeCPUCoresCapacity_UsesAllocatable(t *testing.T) {
	spy := newSpyPromClient()
	querier := newTestQuerier(spy)
	q := captureQuery(t, spy, func() { querier.QueryNodeCPUCoresCapacity(testStart, testEnd) })

	if !strings.Contains(q, "kube_node_status_capacity") {
		t.Errorf("expected metric 'kube_node_status_capacity' in query, got:\n%s", q)
	}
	if !strings.Contains(q, `resource="cpu"`) {
		t.Errorf("expected resource=\"cpu\" selector in query, got:\n%s", q)
	}
...
func TestQueryNodeRAMBytesCapacity_UsesAllocatable(t *testing.T) {
	spy := newSpyPromClient()
	querier := newTestQuerier(spy)
	q := captureQuery(t, spy, func() { querier.QueryNodeRAMBytesCapacity(testStart, testEnd) })
...
// --- P4: PV capacity uses KSM kube_persistentvolume_capacity_bytes ---

func TestQueryPVActiveMinutes_UsesOTelMetric(t *testing.T) {
	spy := newSpyPromClient()
	querier := newTestQuerier(spy)
	q := captureQuery(t, spy, func() { querier.QueryPVActiveMinutes(testStart, testEnd) })

	if !strings.Contains(q, "kube_persistentvolume_capacity_bytes") {
		t.Errorf("expected metric 'kube_persistentvolume_capacity_bytes' in query, got:\n%s", q)
	}
	if strings.Contains(q, "label_replace") {
		t.Errorf("unexpected label_replace in query (KSM already has native k8s_persistentvolume_name), got:\n%s", q)
	}
	if !strings.Contains(q, "k8s_persistentvolume_name") {
		t.Errorf("expected 'k8s_persistentvolume_name' in query, got:\n%s", q)
	}
}

func TestQueryPVBytes_UsesOTelMetric(t *testing.T) {
	spy := newSpyPromClient()
	querier := newTestQuerier(spy)
	q := captureQuery(t, spy, func() { querier.QueryPVBytes(testStart, testEnd) })
...
// --- P6: PVC bytes requested uses KSM kube_persistentvolumeclaim_resource_requests_storage_bytes ---

func TestQueryPVCBytesRequested_UsesOTelMetric(t *testing.T) {
	spy := newSpyPromClient()
	querier := newTestQuerier(spy)
	q := captureQuery(t, spy, func() { querier.QueryPVCBytesRequested(testStart, testEnd) })

	if !strings.Contains(q, "kube_persistentvolumeclaim_resource_requests_storage_bytes") {
		t.Errorf("expected metric 'kube_persistentvolumeclaim_resource_requests_storage_bytes' in query, got:\n%s", q)
	}
	if !strings.Contains(q, "k8s_persistentvolumeclaim_name") {
		t.Errorf("expected 'k8s_persistentvolumeclaim_name' in query, got:\n%s", q)
	}
}

// --- P7: CPU throttling diagnostic removed ---

func TestCPUThrottlingDiagnosticRemoved(t *testing.T) {
	// Verify that CPUThrottlingDiagnosticMetricID is not in the diagnosticDefinitions map
	if _, exists := diagnosticDefinitions[CPUThrottlingDiagnosticMetricID]; exists {
		t.Error("CPUThrottlingDiagnosticMetricID should not be in diagnosticDefinitions (no OTel equivalent)")
	}
}
