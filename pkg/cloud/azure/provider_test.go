...
func TestGetPVKey_IsDiskCSI(t *testing.T) {
	az := &Azure{}

	// Case 1: Azure Disk CSI
	pvDisk := &clustercache.PersistentVolume{
	...
	keyDisk := az.GetPVKey(pvDisk, nil, "eastus")
	require.True(t, keyDisk.(*azurePvKey).IsDiskCSI, "Expected IsDiskCSI to be true for disk.csi.azure.com")

	// Case 2: Azure File CSI
	pvFile := &clustercache.PersistentVolume{
		Spec: v1.PersistentVolumeSpec{
			PersistentVolumeSource: v1.PersistentVolumeSource{
				CSI: &v1.CSIPersistentVolumeSource{
	...
	keyFile := az.GetPVKey(pvFile, nil, "eastus")
	require.False(t, keyFile.(*azurePvKey).IsDiskCSI, "Expected IsDiskCSI to be false for file.csi.azure.com")

	// Case 3: Nil CSI (Legacy)
	pvLegacy := &clustercache.PersistentVolume{
		Spec: v1.PersistentVolumeSpec{
			PersistentVolumeSource: v1.PersistentVolumeSource{
	...
	keyLegacy := az.GetPVKey(pvLegacy, nil, "eastus")
	require.False(t, keyLegacy.(*azurePvKey).IsDiskCSI, "Expected IsDiskCSI to be false for legacy Azure Disk")
}

func TestGetPVKeyAndFeatures_StorageClassesFromAKSDefaults(t *testing.T) {
	az := &Azure{}
	...
	testCases := []struct {
	...
	}{
		{
			...
			expectedStorageClass: AzureFileStandardStorageClass,
		},
		{
			name:        "azurefile-csi",
			provisioner: "file.csi.azure.com",
		...
		},
		{
			...
			expectedStorageClass: AzureFilePremiumStorageClass,
		},
		{
		...
		},
		{
			...
			expectedStorageClass: AzureDiskStandardSSDStorageClass,
		},
		{
			name:        "managed",
			provisioner: "disk.csi.azure.com",
		...
		},
		{
			...
			expectedStorageClass: AzureDiskPremiumSSDStorageClass,
		},
		{
		...
		t.Run(tc.name, func(t *testing.T) {
			pv := &clustercache.PersistentVolume{
				Spec: v1.PersistentVolumeSpec{
					PersistentVolumeSource: v1.PersistentVolumeSource{
						CSI: &v1.CSIPersistentVolumeSource{
							Driver: tc.provisioner,
						},
					},
					StorageClassName: tc.name,
				},
			}

			key := az.GetPVKey(pv, tc.parameters, "eastus")
			require.Equal(t, "eastus,"+tc.expectedStorageClass, key.Features())
		})
	}
}
