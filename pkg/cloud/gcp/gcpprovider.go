...
func New(cache clustercache.ClusterCache, apiKey string, config *provider.ProviderConfig, region string, projectID string) (*GCP, error) {
	return &GCP{
		Clientset:        cache,
		APIKey:           apiKey,
		Config:           config,
...
func getUsageType(labels map[string]string) string {
	if t, ok := labels[GKEPreemptibleLabel]; ok && t == "true" {
		...
		return "preemptible"
	} else if t, ok := labels[aws.KarpenterCapacityTypeLabel]; ok && t == aws.KarpenterCapacitySpotTypeValue {
		return "preemptible"
	}
	return "ondemand"
}
