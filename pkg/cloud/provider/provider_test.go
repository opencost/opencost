...
// TestNewDOKSProvider_ClientInitialization is a regression test ensuring the
// godo client is wired up from DIGITALOCEAN_ACCESS_TOKEN. Previously,
// callers built a DOKS{} struct literal directly instead of calling
// NewDOKSProvider, so the unexported client field was always nil and
// fetchPricingData always failed with "digitalocean client not initialized".
func TestNewDOKSProvider_ClientInitialization(t *testing.T) {
	t.Setenv("DIGITALOCEAN_ACCESS_TOKEN", "test_token_dop_v1_fake")
...
// TestNewDOKSProvider_NoToken verifies that without a token the client is left
// nil and DownloadPricingData surfaces a clear error instead of panicking.
func TestNewDOKSProvider_NoToken(t *testing.T) {
	...
	if provider.client != nil {
		t.Fatal("expected godo client to be nil when no access token is configured")
	}

	if err := provider.DownloadPricingData(); err == nil {
		t.Fatal("expected an error from DownloadPricingData when no access token is configured")
	}
}