...
func (cm *CostModel) buildPodMap(window kubecost.Window, resolution, maxBatchSize time.Duration, podMap map[podKey]*pod, clusterStart, clusterEnd map[string]time.Time, ingestPodUID bool, podUIDKeyMap map[podKey][]podKey) error {
	...
	for coverage.End().Before(end) {
		...
		for resPods == nil && numTries < maxTries {
			...
			if ingestPodUID {
				queryPods = fmt.Sprintf(queryFmtPodsUID, env.GetPrometheusMetricPrefix(), env.GetPromClusterFilter(), env.GetPromClusterLabel(), durStr, resStr)
			} else {
				queryPods = fmt.Sprintf(queryFmtPods, env.GetPrometheusMetricPrefix(), env.GetPromClusterFilter(), env.GetPromClusterLabel(), durStr, resStr)
			}
