...
func ValidatePrometheus(cli prometheusClient.Client, isThanos bool) (*PrometheusMetadata, error) {
	...
	if len(resUp.Results) == 0 {
		return &PrometheusMetadata{
			Running:            false,
			KubecostDataExists: false,
		}, fmt.Errorf("no running jobs on Prometheus at %s", cli.URL(epQuery, nil).Path)
	}

	for _, result := range resUp.Results {
...
func (cm *CostModel) ComputeCostData(cli prometheusClient.Client, clientset kubernetes.Interface, cp costAnalyzerCloud.Provider, window string, offset string, filterNamespace string) (map[string]*CostData, error) {
	...
	resRAMRequests := resChRAMRequests.Await()
	resRAMUsage := resChRAMUsage.Await()
	resCPURequests := resChCPURequests.Await()
	resCPUUsage := resChCPUUsage.Await()
	resGPURequests := resChGPURequests.Await()
	resPVRequests := resChPVRequests.Await()
	resNetZoneRequests := resChNetZoneRequests.Await()
	resNetRegionRequests := resChNetRegionRequests.Await()
	resNetInternetRequests := resChNetInternetRequests.Await()
	resNormalization := resChNormalization.Await()
...
func findDeletedNodeInfo(cli prometheusClient.Client, missingNodes map[string]*costAnalyzerCloud.Node, window string) error {
	if len(missingNodes) > 0 {
		...
		cpuCostRes := cpuCostResCh.Await()
		ramCostRes := ramCostResCh.Await()
		gpuCostRes := gpuCostResCh.Await()
...
func (cm *CostModel) costDataRange(cli prometheusClient.Client, clientset kubernetes.Interface, cp costAnalyzerCloud.Provider,
	...
	resRAMRequests := resChRAMRequests.Await()
	resRAMUsage := resChRAMUsage.Await()
	resRAMAlloc := resChRAMAlloc.Await()
	resCPURequests := resChCPURequests.Await()
	resCPUUsage := resChCPUUsage.Await()
	resCPUAlloc := resChCPUAlloc.Await()
	resGPURequests := resChGPURequests.Await()
	resPVRequests := resChPVRequests.Await()
	resPVCAlloc := resChPVCAlloc.Await()
	resPVHourlyCost := resChPVHourlyCost.Await()
	resNetZoneRequests := resChNetZoneRequests.Await()
	resNetRegionRequests := resChNetRegionRequests.Await()
	resNetInternetRequests := resChNetInternetRequests.Await()
	resNSLabels := resChNSLabels.Await()
	resPodLabels := resChPodLabels.Await()
	resServiceLabels := resChServiceLabels.Await()
	resDeploymentLabels := resChDeploymentLabels.Await()
	resStatefulsetLabels := resChStatefulsetLabels.Await()
	resDaemonsets := resChDaemonsets.Await()
	resNormalization := resChNormalization.Await()
