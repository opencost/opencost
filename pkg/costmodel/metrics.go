...
func SetOTelLabels(clusterID, clusterLabel string) {
	...
	// metricLabelVolumeName intentionally stays "volumename" in OTel mode.
	// This label represents the PV name value (e.g. "pvc-abc123") as used by
	// kube-state-metrics (kube_persistentvolumeclaim_info.volumename). It is a
	// KSM convention, not an OTel-renamed label, so it must not get a k8s_ prefix.
