...
func getRegisteredPlugins(configDir string, execDir string) (map[string]*plugin.Client, error) {
	...
	for name, config := range pluginNames {
		...
		if _, err := os.Stat(file); err != nil {
			msg := fmt.Errorf("error reading executable for %s plugin. Plugin executables must be in %s and have name format <plugin name>.ocplugin.<os>.<opencost binary archtecture (arm64 or amd64)>", name, execDir)
