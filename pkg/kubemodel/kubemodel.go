...
func (km *KubeModel) ComputeKubeModelSet(start, end time.Time) (*kubemodel.KubeModelSet, error) {
	...
	// 2.16 Compute Network Traffic
	err = km.computeNetworkTraffic(kms, start, end)
	if err != nil {
		kms.Error(err)
	}
...
func (km *KubeModel) computeNetworkTraffic(kms *kubemodel.KubeModelSet, start, end time.Time) error {
	grp := source.NewQueryGroup()
	metrics := km.ds.Metrics()
	...
	appendDetail := func(uid string, dir kubemodel.TrafficDirection, tt kubemodel.TrafficType, endpoint string, bytes float64) {
		if _, ok := kms.Pods[uid]; !ok {
			return
		}
		kms.PodNetworkTrafficDetails[uid] = append(kms.PodNetworkTrafficDetails[uid], &kubemodel.NetworkTrafficDetail{
			PodUID:           uid,
	...
	for _, res := range netZoneGiBResult {
		...
		appendDetail(res.UID, kubemodel.TrafficDirectionEgress, kubemodel.TrafficTypeCrossZone, "", res.Data[0].Value*gibToBytes)
	}
	...
	for _, res := range netRegionGiBResult {
		...
		appendDetail(res.UID, kubemodel.TrafficDirectionEgress, kubemodel.TrafficTypeCrossRegion, "", res.Data[0].Value*gibToBytes)
	}
	...
	for _, res := range netInternetGiBResult {
		...
		appendDetail(res.UID, kubemodel.TrafficDirectionEgress, kubemodel.TrafficTypeInternet, "", res.Data[0].Value*gibToBytes)
	}
	...
	for _, res := range netNatGatewayGiBResult {
		...
		appendDetail(res.UID, kubemodel.TrafficDirectionEgress, kubemodel.TrafficTypeNatGateway, "", res.Data[0].Value*gibToBytes)
	}
	...
	for _, res := range netInternetServiceGiBResult {
		...
		appendDetail(res.UID, kubemodel.TrafficDirectionEgress, kubemodel.TrafficTypeInternet, res.Service, res.Data[0].Value*gibToBytes)
	}
	...
	for _, res := range netZoneIngressGiBResult {
		...
		appendDetail(res.UID, kubemodel.TrafficDirectionIngress, kubemodel.TrafficTypeCrossZone, "", res.Data[0].Value*gibToBytes)
	}
	...
	for _, res := range netRegionIngressGiBResult {
		...
		appendDetail(res.UID, kubemodel.TrafficDirectionIngress, kubemodel.TrafficTypeCrossRegion, "", res.Data[0].Value*gibToBytes)
	}
	...
	for _, res := range netInternetIngressGiBResult {
		...
		appendDetail(res.UID, kubemodel.TrafficDirectionIngress, kubemodel.TrafficTypeInternet, "", res.Data[0].Value*gibToBytes)
	}
	...
	for _, res := range netNatGatewayIngressGiBResult {
		...
		appendDetail(res.UID, kubemodel.TrafficDirectionIngress, kubemodel.TrafficTypeNatGateway, "", res.Data[0].Value*gibToBytes)
	}
	...
	for _, res := range netInternetServiceIngressGiBResult {
		...
		appendDetail(res.UID, kubemodel.TrafficDirectionIngress, kubemodel.TrafficTypeInternet, res.Service, res.Data[0].Value*gibToBytes)
	}
