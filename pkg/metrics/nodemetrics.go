...
func newKubeNodeStatusCapacityMetric(fqname, node, resource, unit string, uid string, value float64) KubeNodeStatusCapacityMetric {
...
func (nam KubeNodeLabelsMetric) Write(m *dto.Metric) error {
	...
	labels = append(labels, &dto.LabelPair{Name: &nodeString, Value: &nam.node},
		&dto.LabelPair{Name: toStringPtr("uid"), Value: &nam.uid})
	m.Label = labels
	return nil
}
