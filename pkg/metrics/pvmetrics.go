...
func (kpvcb KubePVCollector) Collect(ch chan<- prometheus.Metric) {
	...
	for _, pv := range pvs {
		if _, disabled := disabledMetrics["kube_persistentvolume_status_phase"]; !disabled {
			...
			if phase != "" {
				...
				for _, p := range phases {
					ch <- newKubePVStatusPhaseMetric("kube_persistentvolume_status_phase", pv.Name, string(pv.UID), p.n, boolFloat64(p.v))
				}
			}
		}
		...
		if _, disabled := disabledMetrics["kube_persistentvolume_capacity_bytes"]; !disabled {
			...
			uid := string(pv.UID)
			m := newKubePVCapacityBytesMetric("kube_persistentvolume_capacity_bytes", pv.Name, uid, float64(storage.Value()))
			ch <- m
		}
		...
		if _, disabled := disabledMetrics["kubecost_pv_info"]; !disabled {
			...
			uid := string(pv.UID)
			m := newKubecostPVInfoMetric("kubecost_pv_info", pv.Name, storageClass, providerID, uid, float64(1))
			ch <- m
		}
	}
}
